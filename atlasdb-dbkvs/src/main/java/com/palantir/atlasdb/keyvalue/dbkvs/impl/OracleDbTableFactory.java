...
import com.palantir.atlasdb.keyvalue.dbkvs.impl.UnbatchedDbReadTable;
...
public class OracleDbTableFactory implements DbTableFactory {
    ...
    public DbReadTable createRead(String tableName, ConnectionSupplier conns) {
        ...
        switch (tableSize) {
        case OVERFLOW:
            queryFactory = new OracleOverflowQueryFactory(tableName, migrationState);
            break;
        case RAW:
            queryFactory = new OracleRawQueryFactory(tableName);
            break;
        default:
