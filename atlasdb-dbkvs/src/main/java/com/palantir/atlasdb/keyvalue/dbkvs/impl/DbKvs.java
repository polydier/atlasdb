...
public class DbKvs extends AbstractKeyValueService {
    ...
    public Map<Cell, Value> getRows(String tableName,
        ...
        return runRead(tableName, new Function<com.palantir.atlasdb.keyvalue.dbkvs.impl.DbReadTable, Map<Cell, Value>>() {
            @Override
    ...
    @Override
    public Map<Cell, Value> get(String tableName, final Map<Cell, Long> timestampByCell) {
        return runRead(tableName, new Function<DbReadTable, Map<Cell, Value>>() {
            @Override
            public Map<Cell, Value> apply(DbReadTable table) {
        ...
        if (!overflowResults.isEmpty()) {
            ...
            try {
                while (overflowIter.hasNext()) {
                    ...
                    values.put(row.getLong("id"), row.getBytes("val"));
                }
            } finally {
                overflowIter.close();
            }
    ...
    @Override
    public Map<Cell, Long> getLatestTimestamps(String tableName, final Map<Cell, Long> timestampByCell) {
        return runRead(tableName, new Function<DbReadTable, Map<Cell, Long>>() {
            @Override
            @SuppressWarnings("deprecation")
    ...
    private void put(String tableName, Map<Cell, byte[]> values, long timestamp, boolean idempotent) {
        final Iterable<List<Entry<Cell, byte[]>>> batches = partitionByCountAndBytes(
                values.entrySet(), INSERT_PARTITION_COUNT, INSERT_PARTITION_BYTES, tableName, getByteSizingFunction());
        runWrite(tableName, new Function<DbWriteTable, Void>() {
            @Override
            public Void apply(DbWriteTable table) {
                for (List<Entry<Cell, byte[]>> batch : batches) {
                    try {
                        ...
                        if (idempotent) {
                            putIfNotUpdate(tableName, table, batch, timestamp, e);
                        } else {
                            throw e;
                        }
                    }
    ...
    @Override
    public void put(String tableName, final Map<Cell, byte[]> values, final long timestamp) throws KeyAlreadyExistsException {
        put(tableName, values, timestamp, true);
    }

    private void putIfNotUpdate(String tableName, DbWriteTable table, List<Entry<Cell, Value>> batch, KeyAlreadyExistsException e) {
        ...
        Map<Cell, Value> results = get(tableName, timestampByCell);
    ...
    private void putIfNotUpdate(String tableName, DbWriteTable table, List<Entry<Cell, byte[]>> batch, final long timestamp, KeyAlreadyExistsException e) {
        ...
        putIfNotUpdate(tableName, table, batchValues, e);
    }

    @Override
    public void putWithTimestamps(String tableName, final Multimap<Cell, Value> cellValues) throws KeyAlreadyExistsException {
        final Iterable<List<Entry<Cell, Value>>> batches = partitionByCountAndBytes(
                cellValues.entries(), INSERT_PARTITION_COUNT, INSERT_PARTITION_BYTES, tableName, getValueSizingFunction());
        runWrite(tableName, new Function<DbWriteTable, Void>() {
            @Override
            public Void apply(DbWriteTable table) {
                for (List<Entry<Cell, Value>> batch : batches) {
                    try {
                        ...
                        putIfNotUpdate(tableName, table, batch, e);
                    }
                }
                return null;
            }
    ...
    public void putUnlessExists(String tableName, Map<Cell, byte[]> values) throws KeyAlreadyExistsException {
        put(tableName, values, AtlasDbConstants.TRANSACTION_TS, false);
    }
    ...
    public void delete(String tableName, Multimap<Cell, Long> keys) {
        ...
        final Iterable<List<Entry<Cell, Long>>> partitions =
                partitionByCountAndBytes(sorted, 10000, getMultiPutBatchSizeBytes(), tableName,
        ...
        runWriteForceAutocommit(tableName, new Function<DbWriteTable, Void>() {
            @Override
            public Void apply(DbWriteTable table) {
    ...
    @Override
    public Map<RangeRequest, TokenBackedBasicResultsPage<RowResult<Value>, byte[]>> getFirstBatchForRanges(final String tableName,
    ...
    public ClosableIterator<RowResult<Value>> getRange(final String tableName,
        ...
        Iterable<RowResult<Value>> rows = new AbstractPagingIterable<RowResult<Value>, TokenBackedBasicResultsPage<RowResult<Value>, byte[]>>() {
            ...
            protected TokenBackedBasicResultsPage<RowResult<Value>, byte[]> getNextPage(
                ...
                return getPage(tableName, newRange, timestamp);
            }
        };
    ...
    private TokenBackedBasicResultsPage<RowResult<Value>, byte[]> getPage(String tableName,
                                                                          final RangeRequest range,
                                                                          final long timestamp) {
        Stopwatch watch = Stopwatch.createStarted();
        try {
            return runRead(tableName, new Function<DbReadTable, TokenBackedBasicResultsPage<RowResult<Value>, byte[]>>() {
                @Override
            ...
            log.info("Call to KVS.getPage on table {} took {} ms.", tableName, watch.elapsed(TimeUnit.MILLISECONDS));
        }
    }
    ...
    public ClosableIterator<RowResult<Set<Long>>> getRangeOfTimestamps(final String tableName,
        ...
        Iterable<RowResult<Set<Long>>> rows = new AbstractPagingIterable<RowResult<Set<Long>>, TokenBackedBasicResultsPage<RowResult<Set<Long>>, byte[]>>() {
            ...
            protected TokenBackedBasicResultsPage<RowResult<Set<Long>>, byte[]> getFirstPage() {
                return getTimestampsPage(tableName, rangeRequest, timestamp);
            }
            ...
            protected TokenBackedBasicResultsPage<RowResult<Set<Long>>, byte[]> getNextPage(
                ...
                byte[] newStartRow = previous.getTokenForNextPage();
                final RangeRequest newRange = rangeRequest.getBuilder().startRowInclusive(newStartRow).build();
                return getTimestampsPage(tableName, newRange, timestamp);
            }
        };
        return ClosableIterators.wrap(rows.iterator());
    }

    private TokenBackedBasicResultsPage<RowResult<Set<Long>>, byte[]> getTimestampsPage(String tableName,
                                                                                        final RangeRequest range,
                                                                                        final long timestamp) {
        Stopwatch watch = Stopwatch.createStarted();
        try {
            return runRead(tableName, new Function<DbReadTable, TokenBackedBasicResultsPage<RowResult<Set<Long>>, byte[]>>() {
                @Override
            ...
            log.info("Call to KVS.getTimestampsPage on table {} took {} ms.", tableName, watch.elapsed(TimeUnit.MILLISECONDS));
        }
    }
    ...
    public void truncateTable(String tableName) {
        runDdl(tableName, new Function<DbDdlTable, Void>() {
            @Override
    ...
    public Set<String> getAllTableNames() {
        return run(new Function<PalantirSqlConnection, Set<String>>() {
            @Override
            public Set<String> apply(PalantirSqlConnection conn) {
                AgnosticResultSet results = conn.selectResultSetUnregisteredQuery(
                        "SELECT table_name FROM pt_metropolis_table_meta");
                Set<String> ret = Sets.newHashSetWithExpectedSize(results.size());
                for (AgnosticResultRow row : results.rows()) {
                    ret.add(row.getString("table_name"));
                }
                return ret;
            }
        });
    ...
    public byte[] getMetadataForTable(String tableName) {
        return runMetadata(tableName, new Function<DbMetadataTable, byte[]>() {
            @Override
    ...
    @Override
    public void putMetadataForTable(String tableName, final byte[] metadata) {
        runMetadata(tableName, new Function<DbMetadataTable, Void>() {
            @Override
    ...
    public Map<String, byte[]> getMetadataForTables() {
        return run(new Function<PalantirSqlConnection, Map<String, byte[]>>() {
            @Override
            @SuppressWarnings("deprecation")
            public Map<String, byte[]> apply(PalantirSqlConnection conn) {
                AgnosticResultSet results = conn.selectResultSetUnregisteredQuery(
                        "SELECT table_name, value FROM pt_metropolis_table_meta");
                Map<String, byte[]> ret = Maps.newHashMapWithExpectedSize(results.size());
                for (AgnosticResultRow row : results.rows()) {
                    ret.put(row.getString("table_name"), row.getBytes("value"));
                }
                return ret;
            }
        });
    ...
    @Override
    public void addGarbageCollectionSentinelValues(String tableName, final Set<Cell> cells) {
        runWrite(tableName, new Function<DbWriteTable, Void>() {
            @Override
            public Void apply(DbWriteTable table) {
    ...
    @Override
    public Multimap<Cell, Long> getAllTimestamps(String tableName, final Set<Cell> cells, final long timestamp) {
        return runRead(tableName, new Function<DbReadTable, Multimap<Cell, Long>>() {
            @Override
            @SuppressWarnings("deprecation")
    ...
    public void checkDatabaseVersion() {
        runDdl("", new Function<DbDdlTable, Void>() {
            @Override
            public Void apply(DbDdlTable table) {
    ...
    private <T> T runMetadata(String tableName, Function<DbMetadataTable, T> runner) {
        ConnectionSupplier conns = new ConnectionSupplier(connections);
        try {
            return runner.apply(dbTables.createMetadata(tableName, conns));
        } finally {
            conns.close();
        }
    }

    private <T> T runDdl(String tableName, Function<DbDdlTable, T> runner) {
        ConnectionSupplier conns = new ConnectionSupplier(connections);
        try {
            return runner.apply(dbTables.createDdl(tableName, conns, systemProperties));
        } finally {
            conns.close();
        }
    }

    private <T> T runRead(String tableName, Function<DbReadTable, T> runner) {
        ConnectionSupplier conns = new ConnectionSupplier(connections);
        try {
            return runner.apply(dbTables.createRead(tableName, conns));
        } finally {
            conns.close();
        }
    }

    private <T> T runWrite(String tableName, Function<DbWriteTable, T> runner) {
        ConnectionSupplier conns = new ConnectionSupplier(connections);
        try {
            return runner.apply(dbTables.createWrite(tableName, conns));
        } finally {
            conns.close();
        }
    }

    private <T> T runWriteForceAutocommit(final String tableName, final Function<DbWriteTable, T> runner) {
        ...
        try {
            ...
            if (!autocommit) {
                return runWriteFreshConnection(conns, tableName, runner);
            } else {
                return runner.apply(dbTables.createWrite(tableName, conns));
            }
        } finally {
     ...
     */
    private <T> T runWriteFreshConnection(final ConnectionSupplier conns, final String tableName, final Function<DbWriteTable, T> runner) {
        ...
        Thread writeThread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    result.set(runner.apply(dbTables.createWrite(tableName, new ConnectionSupplier(Suppliers.ofInstance(freshConn)))));
                } finally {
                    try {
